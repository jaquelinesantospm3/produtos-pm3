...
Feito em Next.js + TypeScript + Tailwind, com Supabase (banco de dados, login e arquivos) e
deploy na Vercel.
...
5. [Configurar o login por e-mail](#5-configurar-o-login-por-e-mail)
...
10. [Como o banco está organizado](#10-como-o-banco-está-organizado)
...
5. Deve aparecer "Success. No rows returned". Pronto: tabelas, regras de acesso, histórico
   automático e a pasta de PDFs foram criados de uma vez.
...
## 5. Configurar o login por e-mail

O acesso é por link mágico: a pessoa digita o e-mail, recebe um link e entra. Sem senha.

1. No Supabase, vá em **Authentication → Sign In / Providers**.
2. Confirme que **Email** está ligado e que **Confirm email** está ligado.
3. Vá em **Authentication → URL Configuration** e preencha:
   - **Site URL**: `http://localhost:3000` por enquanto (depois do deploy você troca pelo
     endereço da Vercel).
   - **Redirect URLs**: adicione `http://localhost:3000/**` e, depois do deploy,
     `https://SEU-PROJETO.vercel.app/**`.

Só e-mails que terminam em `@pm3.com.br` conseguem entrar. Isso é checado em quatro lugares:
na tela de login, no servidor logo depois do clique no link, nas regras do banco (RLS) e num
gatilho que recusa a criação de qualquer conta fora do domínio. Mesmo que alguém tente pela
API, não entra.

> **Sobre o envio de e-mails:** o Supabase gratuito envia poucos e-mails por hora, o suficiente
> para testar. Para o time inteiro usar, vale configurar um SMTP próprio em
> **Project Settings → Authentication → SMTP Settings** (o time de tecnologia da PM3 consegue
> apontar o servidor de e-mail de vocês).
...
3. No Supabase, vá em **Project Settings → API** e copie:
   ...
   Essa chave `anon` pode aparecer no navegador sem problema — quem protege os dados são as
   regras do banco. **Nunca** use aqui a chave `service_role`.
...
5. Abra <http://localhost:3000>, digite seu e-mail @pm3.com.br e entre pelo link que chegar.

---

## 7. Subir o código para o GitHub

Se você tem o GitHub CLI (`gh`) configurado, dentro da pasta do projeto:

```bash
gh repo create produtos-pm3 --private --source . --remote origin --push
```

Se preferir pelo site: crie um repositório vazio no GitHub e rode:

```bash
git remote add origin https://github.com/SUA-CONTA/produtos-pm3.git
```
...
6. Copie o endereço que a Vercel gerou (algo como `https://produtos-pm3.vercel.app`) e volte
   ao Supabase, em **Authentication → URL Configuration**:
   - troque a **Site URL** por esse endereço;
   - adicione `https://produtos-pm3.vercel.app/**` nas **Redirect URLs**.

   Sem esse passo, o link do e-mail leva para o lugar errado.

7. Pronto. Toda vez que você fizer `git push`, a Vercel publica sozinha a versão nova.
...
**Logs** — toda alteração de todo produto, com data, campo, resumo, nome e e-mail de quem
mexeu. Filtra por produto e por pessoa. Ninguém escreve nesses registros à mão: o próprio
banco de dados grava.
...
Quem estiver logado com `@pm3.com.br` pode ver e editar qualquer produto. O nome e o e-mail de
quem está logado assinam automaticamente cada criação e alteração — ninguém digita o próprio
nome. Se o nome deduzido do seu e-mail estiver estranho, ajuste em **Meu perfil** (clique na
bolinha com suas iniciais, no canto superior direito).

---

## 10. Como o banco está organizado
...
| `perfis`                | Nome e e-mail de quem usa o sistema                                   |
...
**PDFs.** Ficam num bucket privado chamado `produtos-pdf`. O link de download é gerado na hora
e vale 5 minutos, então o arquivo não fica exposto na internet.

### Onde mexer no código

```
src/app/(app)/            telas de quem está logado (catálogo, produto, logs, manutenção)
...
**"Esse e-mail não é da PM3."** — é isso mesmo: só `@pm3.com.br` entra.

**O link do e-mail não funciona / cai numa tela de erro.** — quase sempre é a **Redirect URL**
faltando no Supabase (passo 5 ou 8.6). Confira se o endereço da Vercel está lá com `/**` no
final.

**O e-mail não chega.** — veja a caixa de spam. Se estiver testando muito, o limite do plano
gratuito do Supabase pode ter estourado; espere uma hora ou configure o SMTP próprio.

**"Sua sessão expirou".** — saia e entre de novo pelo link.
